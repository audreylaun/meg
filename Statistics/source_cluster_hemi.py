...
def get_labels(src):
    ...
    frontal_labels_old = ["Frontal",
                          [f"G_and_S_frontomargin-{hemi}", f"G_and_S_precentral-{hemi}", f"G_front_middle-{hemi}",
                           ...
                           f"S_subcentral_post-{hemi}", f"S_front_sup-{hemi}", f"G_pariet_inf-Supramar-{hemi}",
                           f"G_pariet_inf-Angular-{hemi}", f"Lat_Fis-post-{hemi}", f"G_temp_sup-Plan_tempo-{hemi}",
                           f"S_interm_prim-Jensen-{hemi}", f"G_and_S_subcentral-{hemi}"]]
    ...
    for i in frontal_labels_1:
        labels_front.append(i)
    for i in frontal_labels_2:
        labels_front.append(i)
    for i in frontal_labels_3:
        labels_front.append(i)

    for i in labels_front:
        if i.name not in [lab.name for lab in labels]:
            labels.append(i)
...
for current_task in Tasks:
    ...
    f=open(OUT + 'Results/%s/%s_results_table_all.txt' %(current_task,hemi), 'w')
    f.write('Model: %s, N=%s\n' %(res.x, len(subjects)))
    ...
    for i in range(len(ix_sign_clusters)):
        ...
        activation.save(OUT+'Results/%s/%s_cluster%s_timecourse_all_(%s-%s).png' %(current_task, hemi, i+1, tstart, tstop))
        activation.close()
        ...
        activation.save(OUT+'Results/%s/%s_cluster%s_timecourse_all_all_(%s-%s).png' %(current_task, hemi, i+1, tstart, tstop))
        activation.close()

            # 2) Brain
        brain = eelbrain.plot.brain.cluster(cluster.mean('time'))
        brain.save_image(OUT+'Results/%s/%s_cluster%s_brain_all_(%s-%s)_Task=%s.png' %(current_task, hemi, i+1, tstart, tstop, current_task))
        brain.close()
